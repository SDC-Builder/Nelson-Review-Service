...
const generate = (id) => (JSON.stringify({
  reviews: reviewsGenerator(randomNumber(3)),
}));
...
(async () => {
  ...
  for (let id = 1; id <= 10000000; id++) {
    ...
    writer.write({
      courseID: id,
      // remove random
      // reviews: JSON.stringify(reviewsGenerator(randomNumber(2))[0])
      starCount: randomNumber(5),
      reviews: JSON.stringify(reviewsGenerator(randomNumber(3)))
